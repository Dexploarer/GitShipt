 ...
 *   5. `bags.createFeeShareConfig` — registers fee claimers, returns configKey.
 *   6. (Day 3) Build, sign, and broadcast the launch transaction.
 *   7. Mark the project `live` and persist `tokenMint`, `bagsLaunchId`,
 *      `bagsConfigKey`.
...
export async function POST(req: Request, ctx: RouteContext): Promise<Response> {
  ...
  try {
    const result = await withIdempotency(idempotencyKey, async () => {
      ...
      const poolWallet = await bags.resolveWallet("github", PLATFORM_GH_USERNAME);
        ...
        feeClaimers: [
          {
            ...
            bps: 9500, // 95% to pool, on-chain platform fee taken via shareFee
          },
        ],
        shareFee: project.platformFeeBps,
      });

      // Step 3: real on-chain launch tx — Day 3. For Day 2 we either:
      //  - Persist the stub state when running stubbed Bags.
      //  - Refuse the launch when canLaunchOnBags() says no.
      //  - Otherwise: skip the broadcast and mark live with a note (the
      //    Day-3 PR will replace this branch with the real signing path).
      ...
      if (isStub) {
        ...
        note =
          "Bags fee-share config registered. On-chain launch transaction will broadcast in the Day-3 release.";
      }
      ...
      if (!updated) {
        throw new LaunchError("update_failed", "Failed to persist launch.", 500);
      }
