...
export async function POST(req: Request): Promise<Response> {
  ...
  const dbp = dbPool();
  const userExists = await dbp.select({ id: users.id }).from(users).where(eq(users.id, userId)).limit(1);
...
async function verifyRepoAdmin(args: {
  ...
  const [account] = await dbp
    ...
    .from(accounts)
    .where(and(eq(accounts.userId, args.userId), eq(accounts.providerId, "github")))
    .limit(1);
