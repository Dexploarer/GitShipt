...
async function requireSession(): Promise<{ userId: string; ip: string | null; userAgent: string | null }> {
...
export async function overrideScoringConfig(input: unknown): Promise<{ ok: true }> {
  ...
  await withIdempotency(parsed.idempotencyKey ?? null, async () => {
    await dbHttp
      .update(projects)
...
export async function retryPayout(input: unknown): Promise<{ ok: true }> {
  ...
  await withIdempotency(parsed.idempotencyKey ?? `payout:retry:${row.id}`, async () => {
    await dbHttp
      .update(payouts)
      .set({
        status: "pending",
...
export async function cancelPayout(input: unknown): Promise<{ ok: true }> {
  ...
  const [row] = await dbHttp
    .select({ id: payouts.id, status: payouts.status, projectId: payouts.projectId })
    .from(payouts)
    .where(eq(payouts.id, parsed.payoutId))
    .limit(1);
  if (!row) throw new Error("payout_not_found");
...
export async function forceSnapshot(input: unknown): Promise<{ ok: true }> {
  ...
  await withIdempotency(parsed.idempotencyKey ?? `snapshot:force:${parsed.projectId}`, async () => {
    const value = {
      projectId: parsed.projectId,
    ...
    await dbHttp
      ...
      .values({ key: `pending.snapshot.${parsed.projectId}`, value, updatedBy: ctx.userId })
      .onConflictDoUpdate({
...
export async function updateFeesBps(input: unknown): Promise<{ ok: true; bps: number }> {
...
export async function toggleKillSwitch(input: unknown): Promise<{ ok: true; enabled: boolean }> {
  ...
  const targetName = parsed.enabled ? "ENABLE KILL SWITCH" : "DISABLE KILL SWITCH";
...
export async function grantRole(input: unknown): Promise<{ ok: true }> {
  ...
  await destructiveAction(
    ...
    async () => {
      await dbHttp.update(users).set({ role: parsed.role }).where(eq(users.id, target.id));
    },
  );
...
export async function retriggerWorkflow(input: unknown): Promise<{ ok: true; runId?: string }> {
...
export async function recomputeLeaderboard(input: unknown): Promise<{ ok: true }> {
...
export async function updatePayoutConfig(input: unknown): Promise<{ ok: true }> {
...
export async function exportAuditCsv(input: unknown): Promise<{ ok: true; csv: string }> {
