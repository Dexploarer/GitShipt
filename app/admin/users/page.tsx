  ...
  return (
    <div className="space-y-4">
      <header className="flex items-end justify-between">
        ...
        <Badge variant="default" size="sm">{rows.length} users</Badge>
      </header>
      ...
      <Card depth="flat" padding="none" className="overflow-hidden">
        <CardHeader className="px-4 pt-4">
          ...
          <CardDescription>Grants are gated by `admin.users.role.grant`.</CardDescription>
        </CardHeader>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-body-sm">
            ...
            <tbody>
              {rows.map((u) => (
                <tr key={u.id} className="border-t border-border/40 hover:bg-surface-elevated/40">
                  <td className="px-4 py-2">
                    <div className="flex min-w-0 items-center gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="truncate text-mono-sm text-fg-muted">{u.email}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-2">
                    <UserManagePanel userId={u.id} userName={u.name} role={u.role} />
                  </td>
                </tr>
              ))}
            </tbody>
...
function RoleBadge({ role }: { role: string }) {
  switch (role) {
    case "super_admin":
      return <Badge variant="danger" size="sm">super_admin</Badge>;
    case "admin":
      return <Badge variant="warning" size="sm">admin</Badge>;
    case "moderator":
      return <Badge variant="info" size="sm">moderator</Badge>;
    default:
