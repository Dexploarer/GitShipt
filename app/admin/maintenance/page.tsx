...
export default async function AdminMaintenancePage() {
  ...
  await requirePermission("platform.maintenance", { userId: session.user.id });

  const ks = await getPlatformConfigValue<{ enabled?: boolean; reason?: string; toggledBy?: string }>(
    "kill_switch.global",
  );
  const banner = await getPlatformConfigValue<{ message?: string; visible?: boolean }>("banner.global");
        ...
        <Card depth="raised" padding="default" className="border-danger/30">
          ...
          <div className="mt-3 space-y-3">
            <div className="flex items-center justify-between">
              ...
              {ks?.enabled ? (
                <Badge variant="danger" dot>ENABLED</Badge>
              ) : (
                <Badge variant="success" dot>disabled</Badge>
              )}
            </div>
            {ks?.reason ? (
              <p className="rounded-md border border-border/40 bg-surface-elevated/50 px-3 py-2 text-body-sm text-fg-secondary">
                <span className="text-label-sm text-fg-muted">last reason: </span>
                {ks.reason}
              </p>
            ) : null}
