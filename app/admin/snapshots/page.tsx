...
import { formatRelativeTime, formatSol, formatAddress } from "@/lib/format";
      ...
      <header className="flex items-end justify-between">
        <div>
          ...
          <p className="text-body-sm text-fg-secondary">
            Reproducibility check: re-running scoring on a snapshot's `inputs` should
            yield the same merkle root.
          </p>
        </div>
        <Badge size="sm" variant="default">{rows.length}</Badge>
      </header>
          ...
          <table className="w-full text-left text-body-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((r) => (
                  <tr key={r.id} className="border-t border-border/40 hover:bg-surface-elevated/40">
                    ...
                    <td className="px-4 py-2 text-mono-sm">{formatSol(r.totalFeesLamports, 4)}</td>
                    <td className="px-4 py-2 text-mono-sm">{r.recipientCount}</td>
                    <td className="px-4 py-2 text-mono-sm" title={r.merkleRoot}>
                      {formatAddress(r.merkleRoot, 6, 6)}
                    </td>
                    <td className="px-4 py-2">
                      {r.forced ? <Badge variant="warning" size="sm">forced</Badge> : <span className="text-fg-muted">—</span>}
                    </td>
                  </tr>
                ))
              )}
...
function SnapStatus({ status }: { status: string }) {
  switch (status) {
    ...
    case "frozen":
      return <Badge variant="info" size="sm" dot>frozen</Badge>;
    case "failed":
    ...
    <Card depth="raised" padding="default">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Sparkles className="size-4 text-fg-muted" /> Snapshot {formatAddress(detail.id, 6, 6)}
        </CardTitle>
        <CardDescription>
          {detail.slug} · taken {formatRelativeTime(detail.takenAt)} · formula {detail.formulaVersion}
        </CardDescription>
      </CardHeader>
      <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-3">
        <div className="space-y-1 text-body-sm">
          ...
          <p className="text-caption text-fg-muted">v1.1 will display the recomputed root + match indicator.</p>
        </div>
        <div className="space-y-1 text-body-sm">
          <p className="text-fg-secondary">Total fees</p>
          <p className="text-mono-sm text-fg">{formatSol(BigInt(detail.totalFeesLamports), 4)}</p>
        </div>
        <div className="space-y-1 text-body-sm">
      ...
      <details className="mt-4">
        <summary className="cursor-pointer text-label-md text-fg">Leaderboard JSON</summary>
