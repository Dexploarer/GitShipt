...
import { AlertTriangle, Github, Power, RefreshCcw, Settings2, Sparkles } from "lucide-react";
...
export default async function AdminProjectDetailPage({
  ...
  const { project, ownerName, ownerUsername, scoringConfig, payoutConfig, contributorsCount } = detail;
    ...
    <div className="space-y-4">
      ...
      <section className="grid grid-cols-1 gap-3 lg:grid-cols-3">
        ...
        <Card depth="flat" padding="sm">
          ...
          <ul className="mt-2 space-y-2 px-1.5 text-body-sm">
            ...
            <Stat label="Platform fee" value={`${project.platformFeeBps} bps`} />
            <Stat label="Created" value={formatRelativeTime(project.createdAt)} />
            <Stat
              label="Paused at"
              value={project.pausedAt ? formatRelativeTime(project.pausedAt) : "—"}
            />
            <Stat
              label="Killed at"
              value={project.killedAt ? formatRelativeTime(project.killedAt) : "—"}
            />
          </ul>
        </Card>
      </section>
      ...
      <Card depth="flat" padding="sm">
        <CardHeader className="px-1.5 pt-1">
          ...
          <CardDescription>
            Per-project history of pause/kill/snapshot decisions lives in the global
            audit log. Filter by `targetId={project.id}`.
          </CardDescription>
        </CardHeader>
