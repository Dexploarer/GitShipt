...
export default async function AdminOpsPage() {
  ...
  const [kpis, heartbeats, failed, recentAudit, healthSummary] = await Promise.all([
    getOpsKpis(hotLamports),
    getHeartbeats(),
    getRecentFailedPayouts(10),
    getAuditLogs({ sinceMs: Date.now() - 60 * 60 * 1000, limit: 20 }),
    fetchHealthSummary(),
  ]);
...
async function pingGitHub(): Promise<{ ok: boolean; latencyMs: number | null }> {
  const start = Date.now();
  try {
            ...
            >
              <div className="flex min-w-0 items-center gap-2">
                <span
                  ...
                  aria-hidden
                />
                <span className="truncate text-label-md text-fg">{r.workflow}</span>
              </div>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-border/30">
                ...
                <td className="max-w-[18rem] truncate px-1.5 py-2 text-fg-secondary" title={r.lastError ?? ""}>
                  {r.lastError ?? "—"}
                </td>
              </tr>
            ))}
  ...
  return (
    <Card depth="raised" padding="sm">
      <CardHeader className="px-1.5 pt-1">
        ...
        <CardDescription>30s cache. Detail at /admin/integrations.</CardDescription>
      </CardHeader>
      <dl className="mt-2 space-y-2 px-1.5">
        <HealthRow label="Bags client" ok={summary.bags.ok} latencyMs={summary.bags.latencyMs} />
        <HealthRow label="GitHub API" ok={summary.github.ok} latencyMs={summary.github.latencyMs} />
      </dl>
      <div className="mt-3 flex items-center justify-between px-1.5">
        <Pill variant={summary.bags.ok && summary.github.ok ? "success" : "danger"} size="sm">
