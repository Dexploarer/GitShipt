...
export default async function AdminTreasuryPage() {
  ...
  await requirePermission("platform.treasury.read", { userId: session.user.id });
  ...
  const hotSol = hotPubkey && hasSolanaConnection() ? await fetchHotSol(hotPubkey) : null;

  const solscanBase =
    cluster === "mainnet-beta"
      ? "https://solscan.io/account/"
      : `https://solscan.io/account/`;
  const solscanCluster = cluster === "mainnet-beta" ? "" : `?cluster=${cluster}`;
    ...
    <div className="space-y-4">
      <header>
        ...
        <p className="text-body-sm text-fg-secondary">
          Read-only. Top-ups go cold-treasury → hot-wallet under MFA (manual, v1.1).
        </p>
      </header>
      ...
      <section className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <StatTile
          ...
          value={hotSol == null ? <span className="text-fg-muted">—</span> : `${hotSol.toFixed(4)} SOL`}
        ...
        <StatTile
          label="Cold treasury"
          value={coldAddress ? formatAddress(coldAddress) : <span className="text-fg-muted">unset</span>}
      ...
      <Card depth="raised" padding="default">
        ...
        <div className="mt-3 flex items-center gap-2">
          <Button variant="primary" size="sm" disabled title="Top-up flow ships in v1.1">
            Top up hot wallet
          </Button>
      ...
      <Card depth="flat" padding="default">
        <CardHeader>
          ...
          <CardDescription>
            Cold key never enters Vercel. Top-ups are signed offline and broadcast
            via the operator's hardware wallet.
          </CardDescription>
        </CardHeader>
        <ul className="mt-3 space-y-2 text-body-sm">
          <li className="flex items-center justify-between">
...
async function fetchHotSol(pubkey: string): Promise<number | null> {
  ...
  try {
    ...
    const lamports = await solanaConnection("confirmed").getBalance(new PublicKey(pubkey));
