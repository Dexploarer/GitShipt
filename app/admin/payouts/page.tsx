    ...
    <div className="space-y-4">
      <header className="flex items-end justify-between">
        <div>
          ...
          <p className="text-body-sm text-fg-secondary">Global queue across all projects.</p>
        </div>
        <Badge size="sm" variant="default">{rows.length} rows</Badge>
      </header>
                ...
                rows.map((r) => (
                  <tr key={r.id} className="border-t border-border/40 hover:bg-surface-elevated/40">
                    <td className="px-4 py-2">
                      <Link href={`/admin/projects/${r.projectId}`} className="text-fg hover:underline">
                        {r.projectSlug}
                      </Link>
                    </td>
                    <td className="px-4 py-2 text-mono-sm">{formatSol(r.totalLamports, 4)}</td>
