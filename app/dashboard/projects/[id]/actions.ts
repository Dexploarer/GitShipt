...
export async function pauseProject(
  ...
  await requirePermission("project.pause", { userId, projectId: data.projectId });
  ...
  const result = await withIdempotency(
    data.idempotencyKey ?? `pause:${data.projectId}:${data.pause ? "1" : "0"}:${Date.now()}`,
    async () => {
...
const payoutConfigSchema = z
  ...
  .refine(
    (v) =>
      Math.abs(
        v.tierWeights.reduce((acc, n) => acc + n, 0) - 1,
      ) < 0.001,
...
export async function forceSnapshot(
  ...
  if (!rl.success) {
    throw new Error("Force-snapshot is rate-limited to 1 per hour per project.");
  }
