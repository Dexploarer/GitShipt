...
export async function createAndLaunchAction(
  ...
  const dbp = dbPool();
  const userRow = await dbp.select({ id: users.id }).from(users).where(eq(users.id, userId)).limit(1);
  if (userRow.length === 0) {
    return {
      ok: false,
      error: "user_missing",
    ...
    const result = await withIdempotency<LaunchActionResult>(
      ...
      async () => {
        ...
        if (isStub) {
          ...
          note =
            "Bags fee-share config registered. On-chain launch transaction broadcasts in the Day-3 release.";
        }
...
async function verifyRepoAdmin(args: {
  ...
  try {
    ...
    const message = e instanceof Error ? e.message : "GitHub repo lookup failed.";
    return {
      ok: false,
      code: "github_error",
      message,
