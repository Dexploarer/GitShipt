 ...
 * just the avatar; the popover still works. When no session is present
 * (no `name` and no `email`), renders nothing.
 */
export function SidebarUserCard({
  ...
  const displayName = name ?? (email ? email.split("@")[0] : null) ?? "Account";
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        title={collapsed ? `${displayName}${handle ? ` · ${handle}` : ""}` : undefined}
        className={cn(
      ...
      {open ? (
        ...
        >
          <MenuLink href={profileHref} icon={UserIcon} label="Profile" onSelect={() => setOpen(false)} />
          <MenuLink href="/dashboard" icon={LayoutDashboard} label="My dashboard" onSelect={() => setOpen(false)} />
          <MenuLink href="/dashboard/wallets" icon={Settings} label="Settings" onSelect={() => setOpen(false)} />
          <Divider />
...
function Avatar({
  ...
  if (imageUrl) {
    // Plain <img> — sidebar avatars are tiny, never the LCP, and the source
    // is always a remote (GitHub) URL we don't want next/image config noise for.
