...
export type AppSidebarSurface =
  | { kind: "public" }
  | { kind: "owner-project"; projectId: string; projectName: string; slug: string }
  | {
      kind: "public-project";
      projectId: string;
      projectName: string;
...
function accountGroup(): NavGroup {
  return {
    ...
    items: [
      ...
      { key: "projects", label: "My projects", icon: FolderGit2, href: "/dashboard/projects" },
      ...
      { key: "security", label: "Security", icon: KeyRound, href: "/dashboard/security" },
    ],
  };
}
...
function platformShortcutGroup(): NavGroup {
  return {
    title: "Platform",
    items: [
      { key: "admin", label: "Admin console", icon: ShieldAlert, href: "/admin" },
    ],
  };
}
...
function publicProjectGroups(slug: string, projectId: string, canAdmin: boolean): NavGroup[] {
  const base = `/r/${slug}`;
  const groups: NavGroup[] = [
    {
      ...
      items: [
        ...
        { key: "payouts", label: "Payouts", icon: Coins, href: `${base}/payouts` },
        { key: "snapshots", label: "Snapshots", icon: History, href: `${base}/snapshots` },
        { key: "repository", label: "Repository", icon: Github, href: `${base}/repository` },
        { key: "token", label: "Token", icon: Sparkles, href: `${base}/token` },
        { key: "docs", label: "Docs", icon: BookOpen, href: `${base}/docs` },
      ],
...
function compose(
  ...
  switch (surface.kind) {
    case "public": {
      ...
      return {
        brand: { title: "GitBags", subtitle: "by BAGS.fm", href: "/" },
        returnTo: null,
        groups,
      };
    }
