...
export async function claimBagsFees(args: {
  ...
  for (const txUnknown of transactions) {
    ...
    const tx = txUnknown as InstanceType<typeof VersionedTransaction>;
    if (typeof (tx as { sign?: unknown }).sign !== "function") continue;
    tx.sign([signer]);
...
export async function buildPlan(
  ...
  const contributorIds = ctxJson.snapshot.leaderboard.map((e) => e.contributorId);
...
export async function dispatchRecipient(args: {
  ...
  escrowDays: number;
}): Promise<{ status: "sent" | "escrow" | "skipped" | "failed"; sig?: string }> {
