...
export interface LeaderboardRow {
  ...
  score: number;
  inputs: { mergedPRs: number; commits: number; reviews: number; issues: number; netLines: number };
...
export async function getProjectLeaderboard(
  ...
  const rows = await dbHttp
    ...
    .from(contributors)
    .leftJoin(contributorClaims, eq(contributorClaims.contributorId, contributors.id))
    .where(
      and(
...
export async function getPoolOverview(header: ProjectHeader): Promise<PoolOverview> {
  ...
  return {
    ...
    sparkline,
    bagsUrl: header.tokenMint ? `https://bags.fm/token/${header.tokenMint}` : null,
    isStub,
  };
}
