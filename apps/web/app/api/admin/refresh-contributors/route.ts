...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    const idemKey =
      ...
      `refresh-contributors:${projectId}:${actorUserId ?? "cron"}`;

    const result = await withIdempotency(
      idemKey,
      async () => {
        const refreshed = await refreshProjectContributors(projectId);
      ...
      { scope: `admin:refresh-contributors:${projectId}` },
    );
